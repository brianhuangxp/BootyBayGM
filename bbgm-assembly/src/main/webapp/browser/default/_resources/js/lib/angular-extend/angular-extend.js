...
//        angular.injector(['ghCommon']).get("commonService").enableLogDebug();  start log
    ...
    (function (angular) {
        ...
        angularExtend.provider('routeResolver', function () {
            ...
            function loadJs($log, appendVersion, jsComp, onSuccess) {
                if(config.isHtml5Mode){
                    jsComp = jsComp.indexOf("/" + config.appPath) == 0 ? jsComp : "/" + config.appPath + "/" + jsComp;
                }
            ...
            function loadCss($log, appendVersion, css, cssPath, onSuccess) {
                ...
                if(config.isHtml5Mode){
                    css = css.indexOf("/" + config.appPath) == 0 ? css : "/" + config.appPath + "/" + css;
                }
            ...
            this.route = (function (routeConfig) {
                function resolve(componentName, path, isLoadCss) {
                    path = path || '';
                    var templateUrl = self.appendVersion(routeConfig.geViewDirectory() + path + componentName + getExtensionName(getHtmlSuffix()));
                    return {
                        templateUrl: config.isHtml5Mode ? "/" + config.appPath + "/" + templateUrl : templateUrl,
                        ...
                        return function (scope, $element, $attr, ctrl) {
                            scope.$watch(optionExp, function ngIncludeWatchAction() {
                                ...
                                if (componentOptions && componentOptions.name) {
                                    ...
                                    if(config.isHtml5Mode){
                                        templateUrl = templateUrl.indexOf("/" + config.appPath) == 0 ? templateUrl : "/" + config.appPath + "/" + templateUrl;
                                        jsURL = jsURL.indexOf("/" + config.appPath) == 0 ? jsURL : "/" +config.appPath + "/" + jsURL;
                                    }
        ...
        angularExtend.provider('loader', function () {
            this.$get = function () {
                ...
                function showLoader() {
                    ...
                    loaderHtml.push('         <img src="/' + config.appPath + '/resources/img/panel/loader_blue.gif" alt=""><div>Loading...</div>');
            ...
            return{
                send: function (option) {
                    ...
                    var requestData = {
                        ...
                        url: option.url.indexOf("/" + config.appPath) == 0 ? option.url : '/' + config.appPath + '/' + option.url,
                        method: method,
